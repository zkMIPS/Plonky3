...
mod tests {
    ...
    type Perm = Poseidon2<F, Poseidon2ExternalMatrix<HLMDSMat4>, DiffusionMatrixBabybear, 16, 7>;
    ...
    fn commit_single_1x8() {
        let perm = Perm::new_from_rng(8, Poseidon2ExternalMatrix::new(HLMDSMat4), 22, DiffusionMatrixBabybear, &mut thread_rng());
    ...
    fn commit_either_order() {
        ...
        let perm = Perm::new_from_rng(8, Poseidon2ExternalMatrix::new(HLMDSMat4), 22, DiffusionMatrixBabybear, &mut rng);
        let hash = MyHash::new(perm.clone());
        let compress = MyCompress::new(perm);
